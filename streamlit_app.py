...
st.title("Dr. Z.C.M.")
st.title('Prediction of hematoma caused by puncture of torsion artery based on artificial intelligence')  # 算法名称 and XXX
...
def do_processing():
    ...
    # 准备模型
    mlpc = MLPClassifier(hidden_layer_sizes=16, activation='logistic', solver='adam', momentum=0.2,
    ...
    with col1:
        ...
        msg = model_print(model_result, "MLP Classifier - Train")
        st.write(msg)
        # 训练画图
        # fig_train = plt_roc_auc([
        #     (model_result, 'MLP Classifier',),
        # ], 'Train ROC')
        # st.pyplot(fig_train)
    # 模型测试、显示结果
    ...
    with col2:
        ...
        msg = model_print(model_test_result, "MLP Classifier - Test")
        st.write(msg)
        # 测试画图
        # fig_test = plt_roc_auc([
        #     (model_test_result, 'MLP Classifier',),
        # ], 'Validate ROC')
        # st.pyplot(fig_test)
...
def do_predict():
    ...
    pr = pr.astype(np.int)
    st.markdown(r"$\color{red}{MLP Classifier}$ $\color{red}{Predict}$ $\color{red}{result}$ $\color{red}{" + str(
